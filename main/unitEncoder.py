...
class UnitEncoder(nn.Module):
    ...
    # unit_title_text               : [batch_size, max_history_num, max_title_length] 사용자 히스토리 명령들의 제목
    # unit_title_mask               : [batch_size, max_history_num, max_title_length]
    # unit_content_text             : [batch_size, max_history_num, max_content_length] 사용자 히스토리 명령들의 본문
    # unit_content_mask             : [batch_size, max_history_num, max_content_length]
    # unit_time_text                : [batch_size, max_history_num, max_time_length] 사용자 히스토리 명령들의 시간
    ...
    # unit_history_category_mask    : [batch_size, category_num] 사용자가 읽은 카테고리
    # unit_history_category_indices : [batch_size, max_history_num] 히스토리 명령의 카테고리 인덱스
    # unit_embedding                : [batch_size, unit_embedding_dim] (옵션) 사용자 ID 임베딩
    # candidate_report_representation : [batch_size, candidate_num, report_embedding_dim] 후보 명령들의 임베딩
    # Output
    # unit_representation           : [batch_size, candidate_num, report_embedding_dim] 각 후보 명령에 대한 사용자 벡터
    def forward(self, unit_dept, unit_pos, unit_rank, unit_unit, unit_title_text, unit_title_mask, unit_content_text, unit_content_mask, unit_time_text, unit_time_mask, unit_history_category, \
...
class MHSA(UnitEncoder):
    ...
    def forward(self, unit_dept, unit_pos, unit_rank, unit_unit, unit_title_text, unit_title_mask, unit_content_text, unit_content_mask, \
                unit_time_text, unit_time_mask, unit_history_category, unit_history_mask, unit_history_graph, unit_history_category_mask, \
                unit_history_category_indices, unit_embedding, candidate_report_representation):
        report_num = candidate_report_representation.size(1)  # Number of candidate reports
        # 1. unit history report encoding
        ...
        history_embedding = self.report_encoder(
            ...
            unit_history_category, unit_embedding
        )  # [batch_size, max_history_num, report_embedding_dim]

        # 2. Multi-Head Self-Attention
        h = self.multiheadAttention(
            history_embedding, history_embedding, history_embedding, unit_history_mask
        ...
        # 4. Attention layer to generate unit representation
        unit_representation = self.attention(h).unsqueeze(dim=1).repeat(1, report_num, 1)  # [batch_size, report_num, report_embedding_dim]
...
class ATT(UnitEncoder):
    ...
    def forward(self, unit_dept, unit_pos, unit_rank, unit_unit, unit_title_text, unit_title_mask, unit_content_text, unit_content_mask, unit_time_text, unit_time_mask, unit_history_category, \
        ...
        position_representation = F.relu(
            self.position_affine(self.position_embedding(unit_pos)),
            inplace=True
...
class ATT_noPosition(UnitEncoder):
    def __init__(self, report_encoder: ReportEncoder, config: Config):
...
class LSTUR(UnitEncoder):
    ...
    def forward(self, unit_dept, unit_pos, unit_rank, unit_unit, unit_title_text, unit_title_mask, unit_content_text, unit_content_mask,
        unit_time_text, unit_time_mask, unit_history_category, unit_history_mask, unit_history_graph, unit_history_category_mask, 
        unit_history_category_indices, unit_embedding, candidate_report_representation):
        batch_size = unit_title_text.size(0)
        report_num = candidate_report_representation.size(1)

        if unit_embedding is None:
            unit_embedding = torch.zeros(
                batch_size, self.report_embedding_dim,
                device=unit_title_text.device, dtype=torch.float32
            )

        unit_history_num = unit_history_mask.sum(dim=1, keepdim=False).long()  # [batch_size]
        ...
        # 2) 길이 기준 정렬
        sorted_unit_history_num, sorted_indices = torch.sort(unit_history_num, descending=True)
        _, desorted_indices = torch.sort(sorted_indices, descending=False)
        nonzero_indices = sorted_unit_history_num.nonzero(as_tuple=False).squeeze(dim=1)

        # 3) 히스토리 없는 경우
        if nonzero_indices.size(0) == 0:
            return unit_embedding.unsqueeze(dim=1).expand(-1, report_num, -1)

        index = nonzero_indices[-1]

        # 4) 히스토리 있는 경우
        if index + 1 == batch_size:
            sorted_unit_embedding = unit_embedding.index_select(0, sorted_indices)
            if self.training and self.masking_probability != 1.0:
                sorted_unit_embedding *= torch.bernoulli(
                    torch.empty([batch_size, 1], device=unit_title_text.device).fill_(self.masking_probability)
                )
            ...
            packed_sorted_history_embedding = pack_padded_sequence(
                sorted_history_embedding, sorted_unit_history_num.cpu(), batch_first=True
            )
            _, h = self.gru(packed_sorted_history_embedding, sorted_unit_embedding.unsqueeze(dim=0))
            unit_representation = h.squeeze(dim=0).index_select(0, desorted_indices)

        else:
            non_empty_indices = sorted_indices[:index + 1]
            empty_indices = sorted_indices[index + 1:]

            sorted_unit_embedding = unit_embedding.index_select(0, non_empty_indices)
            if self.training and self.masking_probability != 1.0:
                sorted_unit_embedding *= torch.bernoulli(
                    torch.empty([index + 1, 1], device=unit_title_text.device).fill_(self.masking_probability)
                )

            sorted_history_embedding = history_embedding.index_select(0, non_empty_indices)
            packed_sorted_history_embedding = pack_padded_sequence(
                sorted_history_embedding, sorted_unit_history_num[:index + 1].cpu(), batch_first=True
            )
            _, h = self.gru(packed_sorted_history_embedding, sorted_unit_embedding.unsqueeze(dim=0))

            unit_representation = torch.cat(
                [h.squeeze(dim=0), unit_embedding.index_select(0, empty_indices)],
                dim=0
            ).index_select(0, desorted_indices)

        # 5) 후보 report 개수만큼 확장
        unit_representation = unit_representation.unsqueeze(dim=1).expand(-1, report_num, -1)
        return unit_representation
...
class CROWN(UnitEncoder):
    ...
    def forward(self, unit_dept, unit_pos, unit_rank, unit_unit, unit_title_text, unit_title_mask, unit_content_text, unit_content_mask, unit_time_text, unit_time_mask, unit_history_category, \
        ...
        """
        batch_size = unit_title_text.size(0)                    # 실제로는 BK (배치 B x 후보 사용자 K)
        ...
        # 4. position 정보 기반 표현
        # --------------------------------------------------
        position_representation = F.relu(self.position_affine(self.position_embedding(unit_pos)), inplace=True)
