...
'''
class CandidateAware_ClickedReportAttention(nn.Module):
    def __init__(self, config, report_encoder):
        super(CandidateAware_ClickedReportAttention, self).__init__()
        ...
        self.head_dim = self.report_embedding_dim // self.num_heads

        assert self.report_embedding_dim % self.num_heads == 0, "embedding_dim must be divisible by num_heads"

        self.last_attn_weights = None
        self.log_weights = False

        self.query_proj = nn.Linear(self.topic_embedding_dim, self.report_embedding_dim)
        self.key_proj = nn.Linear(self.topic_embedding_dim, self.report_embedding_dim)
        self.value_proj = nn.Linear(self.report_embedding_dim, self.report_embedding_dim)

        # attention score scaling
        self.scale = self.report_embedding_dim ** 0.5
        self.dropout = nn.Dropout(p=0.2)

        # Gated Residual
        self.gate_proj = nn.Linear(self.report_embedding_dim, self.report_embedding_dim)
        self.layernorm = nn.LayerNorm(self.report_embedding_dim)
    ...
    def forward(self, clicked_report_embeddings, clicked_report_topic_embeddings, candidate_topic_embeddings, mask=None):
        """
        Input 
        clicked_report_embeddings:        [batch_size, max_history_num, report_embedding_dim]   (V)
        clicked_report_topic_embeddings:  [batch_size, max_history_num, topic_embedding_dim]  (K)
        candidate_topic_embeddings:     [batch_size, report_num, topic_embedding_dim]         (Q)
        mask:                           [batch_size, max_history_num] (optional)
        
        Output
        clicked_report_embeddings:        [batch_size, max_history_num, report_embedding_dim]
        """
        B, H, D = clicked_report_embeddings.size()
        ...
        K = self.key_proj(clicked_report_topic_embeddings).view(B, H, self.num_heads, self.head_dim).transpose(1, 2)  # [B, heads, H, d_k]
        V = self.value_proj(clicked_report_embeddings).view(B, H, self.num_heads, self.head_dim).transpose(1, 2)     # [B, heads, H, d_k]
        ...
        V_orig = clicked_report_embeddings  # [B, H, D]
